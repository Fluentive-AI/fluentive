...
const COMMUNICATION_STATUS_TYPES = [
...
const CommunicationStatusFilter: React.FC<CommunicationStatusFilterProps> = ({
  ...
  const toggleAll = () => {
    const allValues = COMMUNICATION_STATUS_TYPES.map(type => type.value);
  ...
  const getSelectedText = () => {
    ...
    if (selectedValues.length === COMMUNICATION_STATUS_TYPES.length) return 'All Statuses';
    
    const selectedLabels = COMMUNICATION_STATUS_TYPES
        ...
        <div className="space-y-2">
          ...
          >
            <div className={cn(
              ...
              selectedValues.length === COMMUNICATION_STATUS_TYPES.length && "bg-brand-600 border-brand-600",
              selectedValues.length > 0 && selectedValues.length < COMMUNICATION_STATUS_TYPES.length && "border-brand-600",
              selectedValues.length === 0 && "border-gray-200"
            )}>
              {selectedValues.length === COMMUNICATION_STATUS_TYPES.length && <Check className="h-4 w-4 text-white" />}
              {selectedValues.length > 0 && selectedValues.length < COMMUNICATION_STATUS_TYPES.length && (
          ...
          <div className="space-y-1 pl-4">
            {COMMUNICATION_STATUS_TYPES.map((type) => {
              return (
                <div 
                  key={type.value} 
